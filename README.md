# 👋 Hello, I'm Tam

Welcome to my GitHub profile! I'm a passionate developer committed to building meaningful software and contributing to the open-source community. 

---

# Latest Blog Posts 
<!-- BLOG-POST-LIST:START -->
- [Deconstructing X &lpar;Twitter&rpar; Media Streaming: Building a High-Performance Video Extraction Engine](https://dev.to/yqqwe/deconstructing-x-twitter-media-streaming-building-a-high-performance-video-extraction-engine-282j)
- [Giải mã kỹ thuật luồng phát trực tuyến của X &lpar;Twitter&rpar;: Xây dựng bộ máy tải video hiệu suất cao với HLS và FFmpeg](https://dev.to/yqqwe/giai-ma-ky-thuat-luong-phat-truc-tuyen-cua-x-twitter-xay-dung-bo-may-tai-video-hieu-suat-cao-voi-ig2)
- [Why Your API Contract Breaks in Production &lpar;And How to Fix It in the Spec&rpar;](https://dev.to/guo_king_5d052f0d094fe6ca/why-your-api-contract-breaks-in-production-and-how-to-fix-it-in-the-spec-1kjh)
- [เจาะลึกวิศวกรรมการสตรีมมิ่งของ X &lpar;Twitter&rpar;: การสร้างระบบดึงวิดีโอประสิทธิภาพสูงด้วย HLS และ FFmpeg](https://dev.to/yqqwe/ecchaaaluekwiswkrrmkaarstriimmingkhng-x-twitter-kaarsraangrabbduengwidiioprasiththiphaaphsuungdwy-hls-aela-ffmpeg-4gn3)
- [X&lpar;Twitter&rpar; 미디어 스트리밍 분석: 고성능 비디오 파싱 엔진 구축과 기술적 도전 과제](https://dev.to/yqqwe/xtwitter-midieo-seuteuriming-bunseog-goseongneung-bidio-pasing-enjin-gucuggwa-gisuljeog-dojeon-gwaje-k9g)
<!-- BLOG-POST-LIST:END -->

---

# Latest stack oveflow activity
<table>
  <tr><th>Title</th><th>Link</th></tr>
  <!-- STACKOVERFLOW:START --><tr><td>Partial.ly API adding application_fee to Stripe PaymentIntent when using direct API Key &lpar;not OAuth&rpar;</td><td>https://stackoverflow.com/questions/79929869/partial-ly-api-adding-application-fee-to-stripe-paymentintent-when-using-direct</td></tr><tr><td>Why does my Corbett Booking website randomly show a server error?</td><td>https://stackoverflow.com/questions/79929847/why-does-my-corbett-booking-website-randomly-show-a-server-error</td></tr><tr><td>I would like to get some advice on this sql connenction with php. We use this in our school. Can you guys help if there is a better way to do it?</td><td>https://stackoverflow.com/questions/79929636/i-would-like-to-get-some-advice-on-this-sql-connenction-with-php-we-use-this-in</td></tr><tr><td>Program only works from one location in filesystem [duplicate]</td><td>https://stackoverflow.com/questions/79929359/program-only-works-from-one-location-in-filesystem</td></tr><tr><td>Custom HTML and CSS not rendering correctly when adding PHP snippet to wp_footer in WordPress [closed]</td><td>https://stackoverflow.com/questions/79928668/custom-html-and-css-not-rendering-correctly-when-adding-php-snippet-to-wp-footer</td></tr><!-- STACKOVERFLOW:END -->
</table>

---


